...
export default function DebitCardScreen({ navigation }: DebitCardScreenProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const text = success
        ? `Card has been ${isLocked ? 'locked' : 'unlocked'}.`
        : `Card failed to ${isLocked ? 'lock' : 'unlock'}.`;
  ...
  const handleSubmitReissue = async () => {
    ...
    const text = success
      ? 'New card has been successfully requested.'
      : 'Request for new card failed.';
    setAlert({ text, success });
  ...
  return (
    <Screen>
      ...
      <Body
        textAlign="center"
        fontWeight="bold"
        style={alert.success ? styles.success : styles.failed}
      >
        {alert.text}
      </Body>
      ...
      {!isLoading && isEmpty(debitCards) && (
        <View style={styles.container}>
          <Button title="Request Card" onPress={handleSubmitRequest} />
        </View>
      )}
        ...
        <View style={styles.container}>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.column}>
              ...
              <Heading5>{capitalize(startCase(toLower(activeCard.status)))}</Heading5>
            </View>
          </View>
