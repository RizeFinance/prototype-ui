...
export default function DebitCardActivationScreen({ navigation, route }: DebitCardActivationScreenProps): JSX.Element {
    ...
    const activationValidationSchema = Yup.object().shape({
        cardLastFourDigits: Yup.string().required('Last 4 Digits is required.')
            .matches(/^\d{4}$/, {excludeEmptyString: true, message: 'Card number must be exactly four digits.'}),
        cvv: Yup.string().required('CVV is required')
            .matches(/^\d{3,4}$/, {excludeEmptyString: true, message: 'CVV must be three or four digits.'}),
        cardExpiry: Yup.string().required('Card Expiration is required.')
            .matches(/^\d{4}-\d{2}$/, {excludeEmptyString: true, message: 'Card Expiration must be in YYYY-MM format.'}),
    });
